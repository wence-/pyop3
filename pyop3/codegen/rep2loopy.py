...
from gem.node import Memoizer, Node, reuse_if_untouched, traversal
from loopy.symbolic import SubArrayRef
from loopy.types import OpaqueType
# from pyop2.datatypes import as_ctypes
from pytools import ImmutableRecord

from pyop3.api import AccessMode
from pyop3.codegen.optimise import index_merger, rename_nodes
from pyop3.codegen.representation import (Accumulate, Argument, BitShift,
                                          ...
                                          LogicalNot, LogicalOr, Materialise,
                                          Max, Min, MultiIndex, NamedLiteral,
                                          PackInst, PreUnpackInst, Product,
                                          RuntimeIndex, Sum, Symbol,
                                          UnpackInst, Variable, When, Zero)
...
class PyOP3KernelCallable(loopy.ScalarCallable):
    """Handles PyOP3 Kernel passed in as a string
    """
...
class PyOP3KernelLookup(object):
    ...
    def __eq__(self, other):
        if isinstance(other, PyOP3KernelLookup):
            return self.name == other.name and self.code == other.code
        return False

    def __call__(self, target, identifier):
        if identifier == self.name:
            return PyOP3KernelCallable(name=identifier, access=self.access)
        return None
...
def generate(builder, wrapper_name=None):
    ...
    if isinstance(kernel.code, loopy.LoopKernel):
        knl = kernel.code
        ...
        wrapper = loopy.register_function_id_to_in_knl_callable_mapper(
            wrapper,
            PyOP3KernelLookup(kernel.name, code, tuple(builder.argument_accesses)))
        preamble = preamble + "\n" + code
...
def statement_functioncall(expr, context):
    ...
    for access, child in zip(expr.access, expr.children):
        ...
        if isinstance(var, pym.Subscript):
            ...
            arg = var
        if access == AccessMode.READ or (isinstance(child, Argument)
...
def expression_indexed(expr, parameters):
    ...
    index = reduce(lambda x, y: pym.Sum((x, y)),
                   [pym.Product((e, m)) for e, m in zip(extents, multiindex)])
